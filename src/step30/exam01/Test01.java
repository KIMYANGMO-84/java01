// 주제 : SQL맵퍼인 Mybatis적용하기 - MemberDao에 적용하기
...
/*  1) mybatis 설계도(설정 파일)를 읽을 때 사용할 InputStream을 준비한다.
    InputStream inputStream = new FileInputStream(절대경로 또는 상대경로);
    예) InputStream inputStram = 
          new FileInputStream("./bin/step30/exam01/mybatis-config.xml")
    
    2)mybatis에게 제공하는 Resources라는 클래스를 사용하면,      
    classpath디렉토리에 있는 파일을 절대 경로나 상대경로보다 
    더 편리하게 파일을 찾을 수 있다.
    패키지 이름만 경로로 지정하면 된다.
*/
    
    //3) mybatis 설계도를 이용하여 SqlSessionFavtory를 준비한다.
    ...
    //5) SQL Mapper파일에 있는 SQL문을 실행한다.
    //  => SQL Mapper파일의 namespace값과 SQL 문의 id값을 결합하여
    //     실행시킬 SQL을 지정하라.
    List<Member> list = 
        sqlSession.selectList("MemberDao.selectList");
    for (Member m : list) {
 ...
 * 4) SQL문을 실행할 mybatis 객체를 준비하라.
 *  => mybatis.org 사이트의 getting started 페이지로 간다.
 *  => SqlSessionDactory 객체를 준비시키는 자바 코드를 복사한다.
 *  => 이 클래스의 main()에 붙여넣는다
 *  => mybatis설정 파일의 경로 정보를 변경한다. 

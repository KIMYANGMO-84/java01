...
public class Exam11 {
  public static void main(String[] args){
    ...
    // - 다만 짤릴 뿐이다.(컴파일가능/데이터 손실)
    System.out.println(3456789.2345f); // 유효자릿수를 벗어나기 때문에 짤린다
    System.out.println(3456789.2345); // 64비트 부동소수점을 의미. 값이 정상적

    // 32비트에 정수에 아무것도 붙이지 않는다. => 정수는 32비트를 주로 사용한다.
    // 64비트 부동소수점에 아무것도 붙이지 않는다.
    // => 부동소수점을 64비트를 주로 사용한다.
  }
}
...
- 결론 => 부동소수점을 2진수로 표현하면 된다.
- IEEE 754 명세에 부동소수점을 2진수로 변환하는 방법이 기술되어 있다.
  현대의 대부분의 컴퓨터는 이 명세에 따라서 부동 소수점을 다룬다.
- 간략한 설명(실제는 좀 더 복잡하다)
예)3.14 = 314 * 10^-2
[24비트(정수값을 저장) = 가수부][8비트(지수값 저장) = 지수부]
[0000 0000 0000 0001 0011 1010]
...
- IEEE 754 명세서
  [가수부(53비트)][지수부(11비트)]
- 2^53 의 값(9007199254740992)을 저장할 수 있다.
...
62비트 부동소수점 => 유효자릿수 15자리 => 배정도(double-precesion) => double

/* 문제 :
 * => 향후 기능이 계속 추가될 수 있다.
 * => 가능한 기존 소스코드를 손대지 않았으면 한다.
 * => 상속?
 *  계속 기능이 추가될 때 마다 서브 클래스를 만든다면,
 *  너무 많은 서브클래스가 생성되는 문제가 발생한다.
 * 해결책 :
 * 각 메서드를 작은 클래스로 정의한다.
...
public class Test {
  ...
  static {
    commandsMap.put("+", new PlusCommand());
    commandsMap.put("-", new MinusCommand());
    commandsMap.put("x", new MultipleCommand());
  }
  public static void main(String[] args) {
    ...
    Command command = commandsMap.get(args[1]);
    if (command == null) {
